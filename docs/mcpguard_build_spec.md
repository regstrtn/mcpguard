...
mcpguard is a lightweight Python tool that intercepts every tool call between an MCP client (Claude, LangChain, Cursor, any agent) and an MCP server (filesystem, shell, database, API). It evaluates each call against YAML-defined security policies and allows, blocks, logs, or requires human approval.
...
| Other governance tools | Vague, early-stage, complex multi-language setups, no clear docs or adoption |
...
| Other governance tools | ~14 | Go+Py | Partial (adapters) | Yes (expr-lang) | YAML | Elastic License, complex multi-adapter arch |
...
Each audit entry includes a `prev_hash` (hash of the previous entry) and `entry_hash` (hash of itself). This creates a linked chain — if any entry is modified or deleted, `verify_chain()` detects it. Implemented simply (~15 lines). The `mcpguard stats` command automatically verifies chain integrity.
...
- [x] Create repo, pyproject.toml, CI (GitHub Actions)
- [x] Implement `Policy`, `Rule`, `PolicyResult` dataclasses
- [x] Implement `PolicyEngine.from_yaml()` — parse YAML into Policy objects
- [x] Implement `PolicyEngine.evaluate()` — pattern matching logic
- [x] Write 15+ unit tests for policy evaluation (test_policy.py)
- [x] Deliverable: `mcpguard validate --config test.yaml` works

### Day 2: Policy Engine Completeness ✅
- [x] Add `prefix`, `not_prefix`, `contains`, `not_contains`, `max_length` rules
- [x] Add priority sorting (first-match-wins)
- [x] Add `enabled` field support
- [x] Implement `mcpguard test --tool X --args '{}'` CLI command
- [x] Write 15 more tests covering edge cases
- [x] Deliverable: Can test any policy offline

### Day 3: MCP Proxy — Basic Pass-through ✅
- [x] Implement `McpProxy` with asyncio subprocess management
- [x] Implement stdio message reading (newline-delimited JSON-RPC)
- [x] Implement transparent pass-through (no policy evaluation yet)
- [x] Test with a real MCP server (use `npx @anthropic/mcp-filesystem`)
- [x] Deliverable: `mcpguard run -- npx @anthropic/mcp-filesystem /tmp` passes all messages

### Day 4: MCP Proxy — Policy Integration ✅
- [x] Wire PolicyEngine into proxy's `_handle_client_message`
- [x] Intercept `tools/call` method, extract name + arguments
- [x] Return JSON-RPC error for DENY'd calls
- [x] Forward ALLOW'd calls to upstream
- [x] Write integration tests (test_proxy.py)
- [x] Deliverable: Can actually block dangerous commands

### Day 5: Audit Logger ✅
- [x] Implement AuditLogger with JSON-lines output
- [x] Wire into proxy
- [x] Add `--log` flag to CLI
- [x] Write tests (test_audit.py)
- [x] Deliverable: Every tool call securely logged

### Day 6: Human Approval + Polish ✅
- [x] Implement ApprovalHandler using /dev/tty
- [x] Wire APPROVE action into proxy
- [x] Create `default.yaml` and `permissive.yaml` presets
- [x] Implement `mcpguard init` command
- [x] Deliverable: Full policy action set working

### Day 7: FastMCP Middleware + Stats ✅
- [x] Implement McpGuardMiddleware for FastMCP
- [x] Implement `mcpguard stats` dashboard using rich tables
- [x] Write middleware tests (test_middleware.py)
- [x] Deliverable: Both proxy and middleware modes complete

### Day 8-9: End-to-End Testing ✅
- [x] Write E2E tests with real MCP servers
- [x] Test with Claude Code configuration (document in examples/)
- [x] Test with Cursor configuration
- [x] Fix all bugs found
- [x] Deliverable: Battle-tested

### Day 10: README + Docs ✅
- [x] Write comprehensive README with quick start
- [x] Write policy-reference.md
- [x] Write example docs (claude-code.md, cursor.md, antigravity.md)
- [x] Add ASCII architecture diagrams
- [x] Deliverable: Someone can use it from README alone
...
|---|---|---|
| **DPR hash chain** | ✅ Included | ~15 lines of code, makes audit trail tamper-evident. Big credibility boost for a security tool. |
...
| **Independent implementation** | ✅ Intentional | If an idea is sound, we implement it in our simpler pure-Python MIT tool regardless of whether other projects have it. |
| **Project location** | `~/Projects/vibecoded/mcpguard/` | Lives alongside other vibecoded projects. |
| **Missing `strict.yaml`** | ✅ Created (Day 7) | Was referenced by tests and CLI but not previously existing. Deny-by-default preset with explicit APPROVE for safe reads. |
| **`__main__.py` module** | ✅ Created (Day 8) | Required for `python -m mcpguard` to work. Was missing, causing silent failures in subprocess tests. |
| **E2E test approach** | Direct proxy integration | Replaced brittle subprocess-based E2E tests with direct `McpProxy` instantiation. More reliable, faster, and easier to debug. |
| **Response scanning** | ✅ Implemented (Day 8) | Scans upstream responses for leaked secrets (AWS keys, private keys, tokens, JWTs). Was a Day 4 TODO. ~15 lines of regex patterns in proxy.py. |
| **Single policy file** | ✅ Consolidated | Replaced 3 separate policy files (default/strict/permissive) with one `mcpguard.yaml` at project root. `policies/` kept as examples gallery. Reduces decision fatigue for new users. |
| **Shadow mode** | ✅ Implemented | `--shadow` flag on `mcpguard run` logs everything but never blocks. DENY → SHADOW_DENY, APPROVE → SHADOW_APPROVE. Lets users observe before enforcing. |
| **Permissive-leaning default** | ✅ Policy redesign | Default policy blocks only genuinely dangerous things (destruction, credential theft, exfiltration). Everything else is logged, not blocked. Don't hinder users. |
| **Policy packs** | ❌ Skipped | Domain-specific policy bundles add complexity. We keep it simple: one file, edit it yourself. |
| **CONTRIBUTING.md** | ✅ Added | Standard contributing guide for onboarding new developers. |
| **Test coverage** | 93 tests total | 57 policy + 21 audit + 11 E2E + 4 middleware. Covers all core modules including shadow mode. |
