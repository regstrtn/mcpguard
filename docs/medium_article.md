...
And now, enter **`mcpguard`**: the standalone firewall designed specifically to inspect, rate-limit, and block tool calls before they hit your machine.
...
Configuring safety layouts looks exactly like configuring network interfaces. You declare allow lists, rate limit buckets, or explicit reject regex matches.
